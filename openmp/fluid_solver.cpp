...
void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c) {
    float tol = 1e-7, max_c, old_x, change, e=1/c,d=a/c;
    int l = 0;
    do {
        max_c = 0.0f;
        #pragma omp parallel reduction(max:max_c) private(old_x, change)
        {
...
void advect(int M, int N, int O, int b, float *d, float *d0, float *u, float *v,
  ...
  for (int k = 1; k <= O; k++) {
	...
	_mm_prefetch(&d0[IX(i0, j0, k0)], _MM_HINT_T0);
	_mm_prefetch(&d0[IX(i0, j0, k1)], _MM_HINT_T0);
